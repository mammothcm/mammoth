...
import org.apache.hadoop.mapred.CacheFile;
import org.apache.hadoop.mapred.CacheOutputStream;
import org.apache.hadoop.mapred.RawBufferedOutputStream;
...
public class RawLocalFileSystem extends FileSystem {
  ...
  public FSDataOutputStream create(Path f, boolean overwrite, boolean createParent)
    throws IOException {
    if (exists(f) && !overwrite) {
      throw new IOException("File already exists:"+f);
    }
    ...
    //return new FSDataOutputStream(new BufferedOutputStream(
    //    new LocalFSFileOutputStream(f, false), bufferSize), statistics);
    return new FSDataOutputStream(new LocalFSFileOutputStream(f, false), statistics);
  }
  @Override
  public FSDataOutputStream create(byte[] b) throws IOException  {    
  	return new FSDataOutputStream(new RawBufferedOutputStream(b), null);
  }
  public FSDataOutputStream create(CacheFile cf) throws IOException  {    
  	return new FSDataOutputStream(new CacheOutputStream(cf), null);
  }  
