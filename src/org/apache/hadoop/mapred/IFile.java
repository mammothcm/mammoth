...
class IFile {
  ...
  public static class Writer<K extends Object, V extends Object> {
    ...
    IFileOutputStream checksumOut = null;
    ...
    public Writer(Configuration conf, FSDataOutputStream out, 
    	...
    	this.writtenRecordsCounter = null;
    	this.rawOut = out;
      this.start = this.rawOut.getPos();
      this.out = out;
      this.keyClass = keyClass;
   //   Log.info("keyClass: " + keyClass);
  ...
  public static class Reader<K extends Object, V extends Object> {
    ...
    ReduceRamManager ramManager = null;
    ...
    public Reader(Configuration conf, ReduceRamManager rm, InputStream in, 
    ...
    public long getPosition() throws IOException {    
    	if (checksumIn == null) {
    		if (in instanceof CacheFile) {
    			return ((CacheFile)in).getPos();
    		} else if (in instanceof FSDataInputStream) {
    			return ((FSDataInputStream)in).getPos();
    		} else if(in instanceof DataInputBuffer){
    			return ((DataInputBuffer)in).getPosition();
    		}    		
    	}
    ...
    throws IOException {
      ...
      if ((dataIn.getLength()-pos) < recordLength) {
        ...
        if ((dataIn.getLength() - dataIn.getPosition()) < recordLength) {
        	Log.info("dataIn: " + dataIn.getPosition() + ", " + dataIn.getLength()
        			+ ", " + recordLength);
  ...
  public static class InMemoryReader<K, V> extends Reader<K, V> {
    ReduceRamManager ramManager;
    TaskAttemptID taskAttemptId;
    
    public InMemoryReader(ReduceRamManager ramManager, TaskAttemptID taskAttemptId,
  ...
  public static class RawMemoryReader<K, V> extends Reader<K, V> {
  	MemoryElement memEle;

  	RawMemoryReader(MemoryElement me) throws IOException {
  		super(null, null, me.getSize(), null, null);
  		memEle = me;
  	}
  	public long getLength() {
  		return fileLength;
  	...
  	public boolean next(DataInputBuffer key, DataInputBuffer value) throws IOException {
        ...
        if (eof) {
  			...
  			boolean hasNext = memEle.nextKeyValue(key, value);
  			if(!hasNext) {
  				eof = true;
  				return false;
  			}
  	...
  	private void dumpOnError() {
      File dumpFile = new File("../output/" + memEle.getTaskID() + ".dump");
      System.err.println("Dumping corrupt map-output of " + memEle.getTaskID() + 
                         " to " + dumpFile.getAbsolutePath());
      try {
        FileOutputStream fos = new FileOutputStream(dumpFile);
        memEle.dumpToFile(fos);
      } catch (IOException ioe) {
        System.err.println("Failed to dump map-output of " + memEle.getTaskID());
      }
    }
