...
abstract class TaskRunner extends Thread {
  ...
  static final String MAPRED_JOB_ADMIN_JAVA_OPTS =
      "mapreduce.admin.job.child.java.opts";
  ...
  private Vector<String> getVMArgs(TaskAttemptID taskid, File workDir,
    ...
    for (int i = 0; i < javaOptsSplit.length; i++) {
    	if (javaOptsSplit[i].startsWith("-Xmx")) {
    		int heap = Integer.parseInt(javaOptsSplit[i].substring(4, javaOptsSplit[i].length()-1));
    		if (t.isMapTask()) {	    		
	    		javaOptsSplit[i] = new String("-Xmx" + heap * conf.getInt("mapred.tasktracker.map.tasks.maximum", 2) 
	    				+ "m");
	    		break;
	    	} /*else {
	    		javaOptsSplit[i] = new String("-Xmx" + heap * conf.getInt("mapred.tasktracker.reduce.tasks.maximum", 2) 
	    				+ "m");
	    	//	LOG.info("------------------" + javaOptsSplit[i]);
	    		break;
	    	}
	    }
    }	*/
  ...
  //cm added
    if (conf.getBoolean("child.jprofiler", false)) {
    	vargs.add(conf.getProfileParams());
    }
  //  LOG.info(conf.getProfileParams());
