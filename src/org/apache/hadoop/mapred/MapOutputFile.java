...
class MapOutputFile {
  ...
  public Path getSpillFile(TaskAttemptID tid, int spillNumber)
      throws IOException {
    return lDirAlloc.getLocalPathToRead(TaskTracker.OUTPUT +"/"+tid+ "/spill"
        + spillNumber + ".out", conf);
  }
  ...
  public Path getSpillFileForWrite(TaskAttemptID tid, int spillNumber, long size)
      throws IOException {
    return lDirAlloc.getLocalPathForWrite(TaskTracker.OUTPUT +"/"+tid+ "/spill"
        + spillNumber + ".out", size, conf);
  }
  /**
  ...
  public Path getSpillIndexFile(TaskAttemptID tid, int spillNumber)
      throws IOException {
    return lDirAlloc.getLocalPathToRead(TaskTracker.OUTPUT +"/"+tid+ "/spill"
        + spillNumber + ".out.index", conf);
  }
  ...
  public Path getSpillIndexFileForWrite(TaskAttemptID tid, int spillNumber, long size)
      throws IOException {
    return lDirAlloc.getLocalPathForWrite(TaskTracker.OUTPUT + "/"+tid+"/spill"
        + spillNumber + ".out.index", size, conf);
  }
  ...
  public void removeAll(TaskAttemptID tid)
      throws IOException {
    conf.deleteLocalFiles(TaskTracker.OUTPUT+"/"+tid);
  }
